...
type Client interface {
	...
	UpsertRun(ctx context.Context, run session.Run) error
	LoadRun(ctx context.Context, runID string) (session.Run, error)
}
...
func (c *client) UpsertRun(ctx context.Context, run session.Run) error {
	if run.RunID == "" {
...
func (c *client) LoadRun(ctx context.Context, runID string) (session.Run, error) {
	if runID == "" {
		return session.Run{}, errors.New("run id is required")
	}
	ctx, cancel := c.withTimeout(ctx)
	defer cancel()
...
func fromRun(run session.Run) runDocument {
	return runDocument{
		RunID:     run.RunID,
		AgentID:   run.AgentID,
...
func (doc runDocument) toRun() session.Run {
	return session.Run{
		RunID:     doc.RunID,
		AgentID:   doc.AgentID,
