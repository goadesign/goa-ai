...
// Gemini's smaller function-schema vocabulary. Definitions without a
// description are rejected, matching the Bedrock provider's convention rather
// than silently sending empty descriptions to Gemini.
func encodeTools(defs []*model.ToolDefinition, canonToProv map[string]string) ([]*genai.Tool, error) {
	...
	for _, def := range defs {
		...
		schema, err := normalizeToolSchema(def.Input.JSONSchema())
		if err != nil {
...
// arguments. It removes only rules and labels Gemini cannot accept; an unknown
// keyword fails instead of being reinterpreted.
