...
* **Registries & Discovery:** `FromRegistry(...)` generates a per-toolset `Discover(ctx, client)` function returning an immutable `runtime/registry.Toolset`. Load it before startup and pass it through the agent's generated `RegistryToolsets` input to definition, client, and registration helpers. Use `runtime/registry.NewClient` with the clustered registry's generated service client; generated HTTP catalog clients require a matching HTTP server. Discovery supports ordinary schema-based tools and does not update active agents. Keep static generated contracts for confirmation, bounded results, continuations, bookkeeping, child workflows, and server-only result data.
