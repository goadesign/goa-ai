...
// EmitToolResult calls the "EmitToolResult" function in
// registrypb.RegistryClient interface.
func (c *Client) EmitToolResult() goa.Endpoint {
	return func(ctx context.Context, v any) (any, error) {
		inv := goagrpc.NewInvoker(
			BuildEmitToolResultFunc(c.grpccli, c.opts...),
			EncodeEmitToolResultRequest,
			nil)
		res, err := inv.Invoke(ctx, v)
		if err != nil {
			// Try to decode a Goa error response detail before falling back to Fault.
