...
type Server struct {
	...
	EmitToolResultH goagrpc.UnaryHandler
...
func New(e *registry.Endpoints, uh goagrpc.UnaryHandler) *Server {
	return &Server{
		...
		EmitToolResultH: NewEmitToolResultHandler(e.EmitToolResult, uh),
...
// NewEmitToolResultHandler creates a gRPC handler which serves the "registry"
// service "EmitToolResult" endpoint.
func NewEmitToolResultHandler(endpoint goa.Endpoint, h goagrpc.UnaryHandler) goagrpc.UnaryHandler {
	if h == nil {
		h = goagrpc.NewUnaryHandler(endpoint, DecodeEmitToolResultRequest, EncodeEmitToolResultResponse)
	}
	return h
}

// EmitToolResult implements the "EmitToolResult" method in
// registrypb.RegistryServer interface.
func (s *Server) EmitToolResult(ctx context.Context, message *registrypb.EmitToolResultRequest) (*registrypb.EmitToolResultResponse, error) {
	ctx = context.WithValue(ctx, goa.MethodKey, "EmitToolResult")
	ctx = context.WithValue(ctx, goa.ServiceKey, "registry")
	resp, err := s.EmitToolResultH.Handle(ctx, message)
	if err != nil {
		return nil, goagrpc.EncodeError(err)
	}
	return resp.(*registrypb.EmitToolResultResponse), nil
}
