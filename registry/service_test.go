...
func TestRetryToolRepublishesExactOverloadedAdmission(t *testing.T) {
	ctx := context.Background()
	pulseClient := mockpulse.NewClient(t)
	resultStream := mockpulse.NewStream(t)
	pulseClient.AddStream(func(string, ...streamopts.Stream) (clientspulse.Stream, error) {
	...
	requestEventID, err := testRedisClient.HGet(
		ctx,
		store.admissionKey(admitted.ToolUseID),
	...
	require.NoError(t, svc.ReportToolCallOverload(ctx, &genregistry.ProviderToolCallClaimPayload{
		...
		ProviderIncarnationID:     testIncarnationA,
		ProviderRegistrationToken: admitted.RegistrationToken,
		CallRegistrationToken:     admitted.RegistrationToken,
	...
	require.NoError(t, err)
	assert.Equal(t, admitted, retried)
	require.Len(t, streams.messages[toolset.Name], 2)
	assert.Equal(t, admitted.RegistrationToken, streams.messages[toolset.Name][1].RegistrationToken)
}
