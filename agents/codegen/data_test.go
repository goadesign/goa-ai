...
import (
	...
	"goa.design/goa-ai/agents/codegen"
...
func TestBuildGeneratorData(t *testing.T) {
	...
	require.Equal(t, filepath.Join("gen", "calc", "agents", "scribe", "tool_specs"), agent.ToolSpecsDir)
...
func TestGenerateProducesFiles(t *testing.T) {
	...
	require.Contains(t, paths, "gen/calc/agents/scribe/tool_specs/types.go")
	require.Contains(t, paths, "gen/calc/agents/scribe/tool_specs/codecs.go")
	require.Contains(t, paths, "gen/calc/agents/scribe/tool_specs/specs.go")
...
func runAgentDesign(t *testing.T) []eval.Root {
	...
	design := func() {
		...
		goaDSL.Service("calc", func() {
			agentsdsl.Agent("scribe", "Doc helper", func() {
				agentsdsl.Uses(func() {
					agentsdsl.Toolset("summarize", func() {
						agentsdsl.Tool("summarize_doc", "Summarize a document", func() {
							agentsdsl.Args(func() {
								goaDSL.Attribute("doc_id", goaDSL.String, "Document identifier")
								goaDSL.Required("doc_id")
							})
						})
					})
				})
				agentsdsl.Exports(func() {
					agentsdsl.Toolset("docs.export", func() {
						agentsdsl.Tool("draft_reply", "Draft a reply", func() {})
					})
				})
