...
type (
	...
	AgentData struct {
		// Expr references the underlying DSL expression for this agent.
		Expr *agentsExpr.AgentExpr
		// Genpkg stores the Go import path to the generated root (module/gen).
		Genpkg string
		// Name is the DSL-provided identifier.
		Name string
		// Description mirrors the DSL description for documentation.
		Description string
		...
		StructName string
		// VarName is a lower-case helper identifier used inside templates.
		VarName string
	...
	ToolsetData struct {
		...
		// AgentToolsImport is the Go import path to the exported tool helpers.
	...
	ToolData struct {
		// Expr is the original tool DSL expression.
		Expr *agentsExpr.ToolExpr
...
func newAgentData(
	...
	varName := codegen.Goify(agentExpr.Name, false)
	...
	agent := &AgentData{
		...
		ToolSpecsPackage:      toolSpecsPackageName,
		ToolSpecsImportPath:   path.Join(agentImport, toolSpecsPackageName),
		ToolSpecsDir:          filepath.Join(agentDir, toolSpecsPackageName),
	}
...
func newToolsetData(
	...
	ts := &ToolsetData{
		...
		SourceServiceImports: buildServiceImportMap(sourceService),
	...
	if kind == ToolsetKindExported {
		...
		ts.AgentToolsImport = path.Join(agent.ImportPath, "agenttools", toolsetSlug)
	...
	if expr.External {
		...
		populateMCPToolset(ts)
	} else {
		for _, toolExpr := range expr.Tools {
			tool := newToolData(ts, toolExpr)
...
func newToolData(ts *ToolsetData, expr *agentsExpr.ToolExpr) *ToolData {
	qualified := expr.Name
	display := expr.Name
	if ts != nil {
		if ts.QualifiedName != "" {
