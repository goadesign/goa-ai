...
const policyDecisionMetadataKey = "policy_decisions"
...
// from the parent run metadata passed explicitly in ToolCallRequest.
...
// generateToolCallID creates a unique identifier for a tool call by combining
// the run ID, sanitized tool name, and a UUID. This provides stable correlation
// keys for streaming events and nested agent progress tracking.
func generateToolCallID(runID, toolName string) string {
	if runID == "" {
		runID = "unknown"
	}
	if toolName == "" {
	...
	return fmt.Sprintf("%s/%s/%s", runID, safeTool, uuid.NewString())
}
...
func toolHandles(calls []planner.ToolCallRequest) []policy.ToolHandle {
...
// with the tool name.
func (r *Runtime) toolMetadata(calls []planner.ToolCallRequest) []policy.ToolMetadata {
...
func filterToolCalls(calls []planner.ToolCallRequest, allowed []policy.ToolHandle) []planner.ToolCallRequest {
	if len(allowed) == 0 {
		return calls
	}
	...
	filtered := make([]planner.ToolCallRequest, 0, len(calls))
	for _, call := range calls {
