...
func TestDefaultAgentToolExecute_TemplatePreferredOverText(t *testing.T) {
	...
	res, err := exec(ctx, planner.ToolCallRequest{Name: "svc.ts.tool", RunID: "run", Payload: map[string]string{"x": "world"}})
...
func TestDefaultAgentToolExecute_UsesTextWhenNoTemplate(t *testing.T) {
	...
	exec := defaultAgentToolExecute(rt, cfg)
	res, err := exec(ctx, planner.ToolCallRequest{Name: "svc.ts.tool", RunID: "run"})
	require.NoError(t, err)
	require.Equal(t, "ok", res.Payload)
	require.Len(t, got, 1)
...
func TestDefaultAgentToolExecute_ErrorsWhenMissingContent(t *testing.T) {
	...
	exec := defaultAgentToolExecute(rt, cfg)
	_, err := exec(ctx, planner.ToolCallRequest{Name: "svc.ts.tool", RunID: "run"})
	require.Error(t, err)
}
