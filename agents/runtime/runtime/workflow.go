...
// the engine.
//
// This implementation is temporary until the workflow codegen implementation lands.
func (r *Runtime) ExecuteWorkflow(ctx engine.WorkflowContext, input RunInput) (RunOutput, error) {
	if input.AgentID == "" {
		return RunOutput{}, errors.New("agent id is required")
	}
	defer r.storeWorkflowHandle(input.RunID, nil)
	reg, ok := r.Agent(input.AgentID)
	if !ok {
		return RunOutput{}, fmt.Errorf("agent %q is not registered", input.AgentID)
	}
	ctrl := interrupt.NewController(ctx)
	reader := r.memoryReader(ctx.Context(), input.AgentID, input.RunID)
	...
	r.publishHook(ctx.Context(), hooks.NewRunStartedEvent(input.RunID, input.AgentID, runCtx, input), seq)
	r.recordRunStatus(ctx.Context(), &input, run.StatusRunning, nil)
	defer r.publishHook(ctx.Context(), hooks.NewRunCompletedEvent(input.RunID, input.AgentID, "success", nil), seq)
	...
	result, err := r.runPlanActivity(ctx, reg.PlanActivityName, reg.PlanActivityOptions, startReq)
	if err != nil {
		r.recordRunStatus(ctx.Context(), &input, run.StatusFailed, map[string]any{"error": err.Error()})
	...
	out, err := r.runLoop(ctx, reg, &input, planInput, result, caps, deadline, nextAttempt, seq, nil, ctrl)
	if err != nil {
		r.recordRunStatus(ctx.Context(), &input, run.StatusFailed, map[string]any{"error": err.Error()})
		return RunOutput{}, err
	}
	r.recordRunStatus(ctx.Context(), &input, run.StatusCompleted, nil)
	return out, nil
}
	...
	for {
		...
		if len(result.ToolCalls) == 0 {
			if result.FinalResponse == nil {
				return RunOutput{}, errors.New("planner returned neither tool calls nor final response")
			}
			r.publishHook(
				ctx,
		...
		if caps.RemainingToolCalls == 0 && caps.MaxToolCalls > 0 {
			return RunOutput{}, errors.New("tool call cap exceeded")
		}
		...
		if r.Policy != nil {
			...
			if decision.DisableTools {
				return RunOutput{}, errors.New("tool execution disabled by policy")
			}
		...
		if len(allowed) == 0 {
			return RunOutput{}, errors.New("no tools allowed for execution")
		}
		...
		if failures(toolResults) > 0 {
			...
			if caps.MaxConsecutiveFailedToolCalls > 0 && caps.RemainingConsecutiveFailedToolCalls <= 0 {
				return RunOutput{}, errors.New("consecutive failed tool call cap exceeded")
			}
...
func collectToolCallIDs(calls []planner.ToolCallRequest) []string {
...
func (r *Runtime) executeToolCalls(
	...
	for i, call := range calls {
		if call.ToolCallID == "" {
			call.ToolCallID = generateToolCallID(runID, call.Name)
			calls[i] = call
		}
