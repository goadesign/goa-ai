...
func TestExecuteToolActivityReturnsErrorAndHint(t *testing.T) {
	rt := &Runtime{toolsets: map[string]ToolsetRegistration{"svc.ts": {Execute: func(ctx context.Context, call planner.ToolCallRequest) (planner.ToolResult, error) {
...
func TestToolsetTaskQueueOverrideUsed(t *testing.T) {
	...
	if ti, ok := wfCtx.lastRequest.Input.(ToolInput); ok {
		...
		t.Fatalf("expected ToolInput in lastRequest.Input")
	}
}
