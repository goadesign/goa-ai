...
func TestEventSequencingMonotonic(t *testing.T) {
	...
	_, err := rt.runLoop(wfCtx, AgentRegistration{ID: input.AgentID, ExecuteToolActivity: "execute", ResumeActivityName: "resume"}, input, base, initial, policy.CapsState{MaxToolCalls: 1, RemainingToolCalls: 1}, time.Time{}, 2, &turnSequencer{turnID: input.TurnID}, nil, nil)
