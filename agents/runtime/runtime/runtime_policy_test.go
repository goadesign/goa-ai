...
func TestPolicyAllowlistTrimsToolExecution(t *testing.T) {
	...
	rt.toolsets = map[string]ToolsetRegistration{"svc.tools": {Execute: func(ctx context.Context, call planner.ToolCallRequest) (planner.ToolResult, error) {
		return planner.ToolResult{Name: call.Name, Payload: map[string]any{"ok": true}}, nil
	}}}
	...
	out, err := rt.runLoop(wfCtx, AgentRegistration{ID: input.AgentID, ExecuteToolActivity: "execute", ResumeActivityName: "resume"}, input, base, initial, policy.CapsState{MaxToolCalls: 5, RemainingToolCalls: 5}, time.Time{}, 2, &turnSequencer{turnID: "turn-1"}, nil, nil)
