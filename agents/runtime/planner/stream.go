...
// ConsumeStream drains the provided streamer, emitting hook events for text and
// thinking chunks via the AgentContext. It returns the aggregated StreamSummary
// so planners can produce a final response or schedule tool calls. Callers are
// responsible for handling ToolCalls in the resulting summary.
func ConsumeStream(ctx context.Context, streamer model.Streamer, agent AgentContext) (StreamSummary, error) {
	...
	for {
		...
		case model.ChunkTypeThinking:
			if chunk.Message.Content == "" || agent == nil {
				continue
			}
			agent.EmitPlannerNote(ctx, chunk.Message.Content, map[string]string{"phase": "thinking"})
		case model.ChunkTypeToolCall:
			if chunk.ToolCall.Name == "" {
				continue
			}
			summary.ToolCalls = append(summary.ToolCalls, ToolCallRequest{
