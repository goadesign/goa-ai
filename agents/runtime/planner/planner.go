...
type (
	...
	// ToolCallRequest schedules a single tool invocation. The runtime validates the
	...
	ToolResult struct {
		// Name identifies the tool that was executed (matches ToolCallRequest.Name).
		Name string

		// Payload carries the tool result payload if successful (e.g., search results,
