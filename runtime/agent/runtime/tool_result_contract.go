...
// validatePersistedToolResult checks the complete tool-result contract when
// stored bytes return to planner or workflow state.
// Failed results do not require the tool to remain registered because their
// shape is independent of generated result codecs.
func validatePersistedToolResult(
	spec *tools.ToolSpec,
	call ToolCall,
	...
	if failure != nil {
		...
		if len(serverData) > 0 {
			return nil, errors.New("failure and server data are both set")
		}
		if bounds != nil {
			return nil, errors.New("failure and bounds are both set")
		}
		if err := planner.ValidateToolFailure(failure); err != nil {
			return nil, fmt.Errorf("invalid failure: %w", err)
		}
		return nil, nil
	}

	canonicalServerData, err := toolserverdata.Apply(spec.CanonicalizeServerData, serverData)
	if err != nil {
		return nil, fmt.Errorf("invalid server data: %w", err)
	}
	if !bytes.Equal(canonicalServerData, serverData) {
		return nil, errors.New("server data is not canonical")
	}
	decoded, err := decodeSuccessfulToolResult(*spec, result)
	if err != nil {
		return nil, err
	}
	if err := validateToolBoundsContract(*spec, call, false, bounds); err != nil {
		return nil, err
	}
	return decoded, nil
}
