...
// the exact model-authored tool call that initiated the external-input pause.
...
func TestRunLoopToolClarificationPreservesCallAndReturnsAnswer(t *testing.T) {
	...
	assistant := wfCtx.lastPlannerCall.Input.Messages[0]
	...
	user := wfCtx.lastPlannerCall.Input.Messages[1]
