...
// completion_tool.go enforces run-scoped completion by one budgeted tool. The
// caller names the required tool in PolicyOverrides; the workflow accepts only
// a successful execution of that tool as completion and returns an error when
// planner finalization, a cap, or a deadline would otherwise end the run.
...
// completionTool returns the run-scoped tool that owns successful completion.
