...
func TestPlannerActivityExecutionOwnsResponseID(t *testing.T) {
	...
	first, err := rt.preparePlannerActivity(context.Background(), input, rt.newRegistryCatalog(AgentDefinition{}), nil, nil, nil)
	require.NoError(t, err)
	second, err := rt.preparePlannerActivity(context.Background(), input, rt.newRegistryCatalog(AgentDefinition{}), nil, nil, nil)
	require.NoError(t, err)
