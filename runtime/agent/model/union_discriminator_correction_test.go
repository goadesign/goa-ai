...
func TestUnionDiscriminatorCorrectionChoicesAndRejections(t *testing.T) {
	...
	definition := correctionToolDefinition(schema, correctionChoiceFields([]tools.FieldPathSegment{tools.FixedField("choice")}, nil))
	for _, test := range []struct{ name, payload, want string }{
		{"missing", `{"choice":{}}`, `Field "choice.kind" is required and must be one of these JSON strings: ["text","count"].`},
		{"number", `{"choice":{"kind":7}}`, `Field "choice.kind" must contain a JSON string from these values: ["text","count"].`},
		{"boolean", `{"choice":{"kind":true}}`, `Field "choice.kind" must contain a JSON string from these values: ["text","count"].`},
		{"null", `{"choice":{"kind":null}}`, `Field "choice.kind" must contain a JSON string from these values: ["text","count"].`},
		{"object", `{"choice":{"kind":{}}}`, `Field "choice.kind" must contain a JSON string from these values: ["text","count"].`},
		{"array", `{"choice":{"kind":[]}}`, `Field "choice.kind" must contain a JSON string from these values: ["text","count"].`},
		{"unknown", `{"choice":{"kind":"submitted-choice","value":7}}`, `Field "choice.kind" must be one of these JSON strings: ["text","count"].`},
		{"empty string", `{"choice":{"kind":""}}`, `Field "choice.kind" must be one of these JSON strings: ["text","count"].`},
		...
		{"undeclared sibling does not select a branch", `{"extra":"submitted-intent","choice":{}}`, `Field "choice.kind" is required and must be one of these JSON strings: ["text","count"].`},
	} {
...
func TestUnionDiscriminatorCorrectionCollections(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			want:    `Field "items.*.kind" is required and must be one of these JSON strings: ["text","count"].`,
		},
		{
			...
			path:    []tools.FieldPathSegment{tools.FixedField("items"), tools.DynamicField{}},
			want:    `Field "items.*.kind" must contain a JSON string from these values: ["text","count"].`,
		},
		{
			...
			want:    `Field "rows.*.*.kind" must be one of these JSON strings: ["text","count"].`,
		},
		{
		...
		t.Run(test.name, func(t *testing.T) {
			rejected := checkToolCorrection(t, correctionToolDefinition(test.schema, correctionChoiceFields(test.path, nil)), test.payload, test.want)
...
func TestUnionDiscriminatorCorrectionOuterBranchCorrelation(t *testing.T) {
	...
	fields := []tools.FieldMetadata{{Path: outer, JSONType: "string", DiscriminatorValues: []string{"nested", "flat"}}}
	...
	definition := correctionToolDefinition(schema, fields)
	for _, test := range []struct{ name, payload, want string }{
		{"selected outer branch", `{"items":[{"mode":"flat","value":7},{"mode":"nested","value":{}}]}`, `Field "items.*.value.kind" is required and must be one of these JSON strings: ["text","count"].`},
		...
		{"missing outer selector", `{"items":[{"value":{}}]}`, `Field "items.*.mode" is required and must be one of these JSON strings: ["nested","flat"].`},
		{"valid nested", `{"items":[{"mode":"nested","value":{"kind":"text","value":"ok"}}]}`, ""},
	} {
		t.Run(test.name, func(t *testing.T) {
			checkToolCorrection(t, definition, test.payload, test.want)
		})
	}
}
...
func TestUnionDiscriminatorCorrectionMetadataAndCompetingErrors(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"identical metadata", baseSchema, `{"choice":{}}`, `Field "choice.kind" is required and must be one of these JSON strings: ["text","count"].`, append(slices.Clone(baseFields), baseFields[1])},
		...
		{"unmatched enclosing branch", baseSchema, `{"choice":{}}`, advertisedToolInputCorrection, correctionChoiceFields(choice, []tools.UnionBranch{{Discriminator: []tools.FieldPathSegment{tools.FixedField("mode")}, Value: "nested"}})},
		{
			...
			`{"choice":{},"other":{}}`, advertisedToolInputCorrection,
		...
		},
		{
			...
			`{"choice":{},"peer":{"name":"x"}}`, advertisedToolInputCorrection,
		...
		},
		{
			...
			`{"choice":{},"peer":{"nested":{"name":"x"}}}`, advertisedToolInputCorrection,
		...
		t.Run(test.name, func(t *testing.T) {
			checkToolCorrection(t, correctionToolDefinition(test.schema, test.fields), test.payload, test.want)
		})
	}
}

func TestUnionDiscriminatorCorrectionSizeBudget(t *testing.T) {
	base := `Field "choice.kind" is required and must be one of these JSON strings: ["text","count"]. Field description: "". Return a replacement tool call with valid arguments.`
	for _, size := range []int{correction.MaxBytes - 1, correction.MaxBytes, correction.MaxBytes + 1} {
		t.Run(fmt.Sprint(size), func(t *testing.T) {
			...
			want := fmt.Sprintf(`Field "choice.kind" is required and must be one of these JSON strings: ["text","count"]. Field description: %q.`, fields[1].Description)
			if size > correction.MaxBytes {
				want = advertisedToolInputCorrection
			}
			schema := fmt.Sprintf(`{"type":"object","properties":{"choice":%s}}`, correctionChoiceSchema)
			checkToolCorrection(t, correctionToolDefinition(schema, fields), `{"choice":{}}`, want)
		})
	}
}
...
func correctionChoiceFields(path []tools.FieldPathSegment, branches []tools.UnionBranch) []tools.FieldMetadata {
	...
	fields := []tools.FieldMetadata{
		{Path: slices.Clone(path), JSONType: "object", Branches: slices.Clone(branches)},
		{Path: discriminator, JSONType: "string", Branches: slices.Clone(branches), DiscriminatorValues: []string{"text", "count"}},
	}
