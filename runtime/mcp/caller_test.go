...
func TestHTTPCallerCallTool(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{`{"ok":true}`}, resp.Content)
...
func TestStdioCallerCallTool(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{expectedTrace}, resp.Content)
...
func TestStdioCallerSendsClientInfo(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{`"operations-agent@2.4.1"`}, resp.Content)
}
...
func TestStdioCallerHandlesServerMessagesWhileToolCallIsPending(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{`"operations-agent@2.4.1"`}, response.Content)
...
func TestNormalizeToolResultReturnsTypedExecutionError(t *testing.T) {
	...
	assert.Equal(t, []string{message, detail}, executionErr.Response.Content)
...
func TestNormalizeToolResultPreservesTextAndStructuredContent(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{"plain result", `{"temperature":22.5}`}, response.Content)
...
func TestNormalizeToolResultDoesNotInferStructuredContentFromText(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{`{"temperature":22.5}`}, response.Content)
...
func TestNormalizeToolResultRejectsUnsupportedContent(t *testing.T) {
	t.Parallel()
	...
	require.EqualError(t, err, `malformed MCP response: unsupported MCP content type "image"`)
}
