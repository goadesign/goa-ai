...
func (p *evalPlan) planPlugin(plan *goagenerator.Plan) error {
	root := serviceDesignRoot(plan.Generation().Roots())
	if root == nil {
		return p.plan(plan.Generation(), nil)
	}
	return p.plan(plan.Generation(), plan.Service(root))
}
...
func (p *evalPlan) plan(generation *goacodegen.Generation, servicePlan *service.Plan) error {
	if p.generation != nil {
	...
	return p.planSuites(root, servicePlan)
}

// planSuites saves each suite and its tool contract.
func (p *evalPlan) planSuites(root *evalexpr.RootExpr, servicePlan *service.Plan) error {
	...
	for _, suite := range root.Suites {
		...
		if servicePlan == nil {
			return errors.New("goa service plan not found for agent evaluation suite")
		}
		var err error
		design, err = agentir.Build(p.generation, servicePlan)
		if err != nil {
			return err
		}
		break
...
// serviceDesignRoot returns the Goa services evaluated by this generation.
func serviceDesignRoot(roots []eval.Root) *goaexpr.RootExpr {
	for _, root := range roots {
		if services, ok := root.(*goaexpr.RootExpr); ok {
			return services
		}
	}
	return nil
...
func (p *evalPlan) generate(files []*goacodegen.File) ([]*goacodegen.File, error) {
	...
	if p.example {
		for _, planned := range p.examples {
			if err := planned.Suite.SuiteImports.Link(); err != nil {
				return nil, err
			}
			if err := planned.Imports.Link(); err != nil {
				return nil, err
			}
	...
	for _, planned := range p.suites {
		if err := planned.SuiteImports.Link(); err != nil {
			return nil, err
		}
...
func planExample(
	...
	planned := &examplePlan{
		...
		PackagePlan: pkg,
		Imports:     goacodegen.NewGeneratedImportPlan(pkg),
	}
	if err := planned.Imports.Require(
		...
		goacodegen.SimpleImport("os"),
		goacodegen.NewImport("eval", "goa.design/goa-ai/eval"),
	); err != nil {
		return nil, err
	}
	if err := planned.Imports.AddGenerated(
		goacodegen.NewImport("geneval"+suite.Package, suite.ImportPath),
	); err != nil {
		return nil, err
	}
