...
type (
	...
	examplePlan struct {
		...
		Imports        *goacodegen.GeneratedImportPlan
	}
	...
	exampleData struct {
		*suiteData
		FileImports           []*goacodegen.ImportSpec
		ExampleHooks          string
...
func planSuite(
	...
	planned := &suitePlan{
		...
		PackagePlan:     pkg,
		SuiteImports:    goacodegen.NewGeneratedImportPlan(pkg),
		ContractImports: goacodegen.NewGeneratedImportPlan(pkg),
		Scenarios:       make([]scenarioPlan, len(suite.Scenarios)),
		Contract:        contract,
	}
	if err := planned.SuiteImports.Require(
	...
	if contract != nil {
		if err := planned.ContractImports.Require(
			goacodegen.SimpleImport("fmt"),
		...
		for _, spec := range contract.Specs {
			if err := planned.ContractImports.AddGenerated(
				goacodegen.NewImport(spec.Package, spec.Path),
			); err != nil {
				return nil, err
			}
		}
...
func planScenario(
	...
	for _, inputType := range newTypes {
		if err := suite.SuiteImports.AddTypeExpressions(inputType.Attribute); err != nil {
			return scenarioPlan{}, err
		}
		// Named objects and unions use pointer parameters. Their public
		// validator reports a missing input before checking any fields.
		if goaexpr.IsObject(inputType.Validator.Reference.Type) ||
			goaexpr.IsUnion(inputType.Validator.Reference.Type) {
			if err := suite.SuiteImports.Require(goacodegen.GoaImport("")); err != nil {
				return scenarioPlan{}, err
			}
		}
		for _, spec := range goacodegen.ValidationRuntimeImports(
			inputType.Attribute,
...
func linkSuiteData(planned *suitePlan, exampleAlias string) *suiteData {
	...
	data := &suiteData{
		...
		Imports:        planned.SuiteImports.Imports(),
...
func linkContractData(planned *suitePlan) *contractData {
	data := &contractData{
		...
		Imports:          planned.ContractImports.Imports(),
	}
	for _, spec := range planned.Contract.Specs {
		data.Specs = append(data.Specs, contractSpecData{
			Alias: planned.PackagePlan.ImportName(spec.Path),
			Path:  spec.Path,
		})
	}
	return data
...
func linkExampleData(planned *examplePlan) *exampleData {
	alias := planned.PackagePlan.ImportName(planned.Suite.ImportPath)
	return &exampleData{
		suiteData:             linkSuiteData(planned.Suite, alias),
		FileImports:           planned.Imports.Imports(),
