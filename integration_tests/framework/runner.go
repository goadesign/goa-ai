...
func regenerateExample(t *testing.T, exampleRoot string) error {
	...
	genCmd.Env = generationEnvironment()
	if out, err := genCmd.CombinedOutput(); err != nil {
		return fmt.Errorf("goa gen failed: %w\n%s", err, string(out))
	}
	// Remove top-level stubs so goa example writes every method with the current
	...
	for _, name := range []string{
		...
		if err := removeExampleFile(filepath.Join(exampleRoot, name)); err != nil {
			return err
		}
	}
	...
	exCmd.Env = generationEnvironment()
	if out, err := exCmd.CombinedOutput(); err != nil {
		return fmt.Errorf("goa example failed: %w\n%s", err, string(out))
	}
	// The fixture supplies its own MCP adapter, so discard the example stub.
	if err := removeExampleFile(filepath.Join(exampleRoot, "mcp_assistant.go")); err != nil {
		return err
	}
...
// generationEnvironment lets Go resolve local modules from an active
// workspace. Without a workspace, generation may update fixture dependencies.
func generationEnvironment() []string {
	environment := os.Environ()
	if workspace := os.Getenv("GOWORK"); workspace != "" && workspace != "off" {
		return environment
	}
	return append(environment, "GOFLAGS=-mod=mod")
}

// removeExampleFile removes one disposable example stub. A missing stub is
// valid because not every design causes goa example to write every file.
func removeExampleFile(path string) error {
	if err := os.Remove(path); err != nil && !errors.Is(err, fs.ErrNotExist) {
		return fmt.Errorf("remove generated example file %s: %w", path, err)
	}
	return nil
}
...
func (r *Runner) ensureInitialized() error {
	...
	if _, _, err := r.executeJSONRPC(methodInitialize, payload, nil, false); err != nil {
		return err
	}
	_, _, err := r.executeJSONRPC("notifications/initialized", nil, nil, true)
	return err
}
...
func (r *Runner) executeJSONRPC(
	...
	for k, v := range headers {
		// Scenario headers beginning with MCP_ configure the fixture process.
		if strings.HasPrefix(k, "MCP_") {
			if err := os.Setenv(k, v); err != nil {
				return nil, nil, fmt.Errorf("set fixture environment %s: %w", k, err)
			}
			continue
		}
		req.Header.Set(k, v)
	...
	if method != methodInitialize && req.Header.Get("MCP-Protocol-Version") == "" {
...
func (r *Runner) executeSSE(
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		closeErr := resp.Body.Close()
		if err := errors.Join(readErr, closeErr); err != nil {
			return nil, fmt.Errorf("read JSON-RPC error response: %w", err)
		}
	...
	if ct := resp.Header.Get("Content-Type"); ct != "" && !strings.HasPrefix(strings.ToLower(ct), "text/event-stream") {
		...
		closeErr := resp.Body.Close()
		if err := errors.Join(readErr, closeErr); err != nil {
			return nil, fmt.Errorf("read unexpected JSON-RPC response: %w", err)
		}
	...
	for time.Now().Before(deadline) {
		...
		if after, ok := strings.CutPrefix(line, "data:"); ok {
			...
			if err := json.Unmarshal([]byte(data), &m); err != nil {
				return events, fmt.Errorf("decode event data: %w", err)
			}
			if cur.Data == nil {
