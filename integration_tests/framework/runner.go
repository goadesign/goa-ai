...
func cleanGeneratedExampleArtifacts(exampleRoot string) error {
	if err := os.RemoveAll(filepath.Join(exampleRoot, "cmd")); err != nil {
		return fmt.Errorf("remove generated cmd tree: %w", err)
	}
	return filepath.WalkDir(exampleRoot, func(path string, d os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		...
		content, readErr := os.ReadFile(path) // #nosec G304 -- test helper reads a generated test file path
		if readErr != nil {
