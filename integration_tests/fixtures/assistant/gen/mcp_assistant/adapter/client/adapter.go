...
func NewEndpoints(
	...
	e.MultiContent = func(ctx context.Context, v any) (any, error) {
		...
		// Call MCP tools/call via transport endpoint (SSE stream)
		streamAny, err := mcpC.ToolsCall()(ctx, &mcpAssistant.ToolsCallPayload{Name: "multi_content", Arguments: args})
		if err != nil {
		...
		stream, ok := streamAny.(*mcpAssistantjsonrpcc.ToolsCallClientStream)
		if !ok {
			return nil, fmt.Errorf("unexpected stream type for multi_content")
		}
		var r *mcpAssistant.ToolsCallResult
		for {
			ev, recvErr := stream.Recv(ctx)
			...
			if recvErr != nil {
				prompt := retry.BuildRepairPrompt("tools/call:multi_content", recvErr.Error(), "{\"count\":1}", "{\"type\":\"object\",\"required\":[\"count\"],\"properties\":{\"count\":{\"type\":\"integer\",\"description\":\"Number of content items to return\"}},\"additionalProperties\":false}")
				return nil, &retry.RetryableError{Prompt: prompt, Cause: recvErr}
			}
			r = ev
		}
