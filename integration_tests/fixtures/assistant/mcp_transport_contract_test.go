// This file verifies that the generated JSON-RPC transport preserves MCP JSON arguments.
...
// the exact JSON object sent by an MCP client.
func TestPromptsGetTransportPreservesArguments(t *testing.T) {
	...
	require.JSONEq(t, `{"code":"x"}`, string(payload.Arguments))
}
