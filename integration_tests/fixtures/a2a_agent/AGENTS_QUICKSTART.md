...
* **Service `test_agent`:**
    * **Agent `test_agent`** (ID: `test_agent.test_agent`):
        * **Mission:** *A test agent for A2A integration tests*
        * **Uses Toolsets:***none*
        * **Exports Toolsets:**
            * `test_tools`
        * **Run Policy:**
...
func main() {
    ...
    {
        cfg := test_agent.TestAgentAgentConfig{
            Planner: &StubPlanner{},
            // We'll add tool configurations here later on.
        }
        if err := test_agent.RegisterTestAgentAgent(context.Background(), rt, cfg); err != nil {
            panic(err)
        }
    }
    ...
    client := test_agent.NewClient(rt)
    out, err := client.Run(
        context.Background(),
        []*model.Message{
...
<details>
<summary><strong>Agent: <code>test_agent</code></strong> (ID: <code>test_agent.test_agent</code>)</summary>

* **Package:** `example.com/a2a_agent/gen/test_agent/agents/test_agent`
* **Directory:** `gen/test_agent/agents/test_agent`
* **Config Struct:** `TestAgentAgentConfig`
* **Register Function:** `RegisterTestAgentAgent(ctx, rt, cfg)`
* **How to Run:**
    ...
    * **Asynchronous (get a handle):**
        ...
        ```
* **Workflow Name:** `test_agent.test_agent.workflow` (Queue: `test_agent_test_agent_workflow`)
