...
* **Service `images`:**
    * **Agent `observer`** (ID: `images.observer`):
        * **Mission:** *Inspects explicitly selected native images.*
        * **Uses Toolsets:**
            * `images.pictures`
...
<details>
<summary><strong>Agent: <code>observer</code></strong> (ID: <code>images.observer</code>)</summary>

* **Package:** `goa.design/goa-ai/internal/testimage/gen/images/agents/observer`
* **Directory:** `gen/images/agents/observer`
* **Config Struct:** `ObserverAgentConfig`
* **Register Function:** `RegisterObserverAgent(ctx, rt, cfg)`
* **How to Run:**
    ...
    * **Asynchronous (get a handle):**
        ...
        ```
* **Workflow Name:** `images.observer.workflow` (Queue: `images_observer_workflow`)

#### Minimal Configuration```go
cfg := observer.ObserverAgentConfig{
    Planner: myPlanner,
}
```
</details>
    ...
    err := toolprovider.Serve(ctx, pulseClient, toolsetID, handler,
        toolprovider.Registration{
            ...
            Renew: func(ctx context.Context, toolset, providerID, incarnationID, expectedToken string) (time.Duration, error) {
                result, err := registryClient.RenewProvider(ctx, &registry.RenewProviderPayload{
                    Name:                      toolset,
                    ProviderID:                providerID,
                    ProviderIncarnationID:     incarnationID,
...
### Agent `observer` Toolsets

* **Tools this agent can USE:**
* **`images.pictures`** 
* **Tool: `pictures.view`**
* *Inspect one selected retained image.*
...
* **Deferred tool search:** Add `Deferred()` inside the consuming `Use` for all static or registry tools, or `Deferred("search", "analyze")` for exact authored local names in a compiled toolset; other tools remain immediately available. Named selections work with local and MCP tools, but not `FromRegistry` or whole registries. Empty, duplicate, unknown, and mixed all/named selections are rejected; repeated `Deferred()` remains valid. Regenerate the consumer after changing its choice. Pass `input.Agent.AdvertisedToolDefinitions()` to model requests; do not implement a planner search tool. OpenAI's adapter searches generated metadata with BM25 and returns native definition results; Claude performs hosted search, including through Bedrock's Messages adapter. Preserve message metadata through history storage and compaction. Unsupported adapters fail explicitly. Regenerate old startup `Discover`/`RegistryToolsets` integrations and replace their dynamic executor wiring with `RegisterRegistry`.
