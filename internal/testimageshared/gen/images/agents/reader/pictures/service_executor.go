...
//
//   exec := pictures.NewReaderPicturesExec(pictures.WithClient(client))

type (
	seCfg struct {
		inspectCaller func(context.Context, any) (any, error)
...
func WithClient(client *images.Client) ExecOpt {
	return execOptFunc(func(c *seCfg) {
		...
		c.inspectCaller = func(ctx context.Context, args any) (any, error) {
			return client.View(ctx, args.(*images.ViewPayload))
		}
...
// WithInspect sets the caller for pictures.inspect.
func WithInspect(f func(context.Context, any) (any, error)) ExecOpt {
	return execOptFunc(func(c *seCfg) {
		c.inspectCaller = f
	})
}
...
// NewReaderPicturesExec returns an executor for the Goa methods used by this toolset.
func NewReaderPicturesExec(opts ...ExecOpt) runtime.ToolCallExecutor {
	var cfg seCfg
	...
	if cfg.inspectCaller == nil {
		panic(fmt.Errorf("service executor missing caller for tool %q", "pictures.inspect"))
	}
	...
	return runtime.ToolCallExecutorFunc(func(ctx context.Context, meta *runtime.ToolCallMeta, call *runtime.ToolCall) (*runtime.ToolExecutionResult, error) {
		...
		case tools.Ident("pictures.inspect"):
			var toolArgs any
			{
				val, err := picturesspecs.InspectPayloadCodec().FromJSON(call.Payload)
				if err != nil {
			...
			if cfg.mapPayload != nil {
				...
				methodIn = picturesspecs.InitInspectMethodPayload(toolArgs.(*picturesspecs.InspectPayload))
			}
			...
			methodOut, err := cfg.inspectCaller(ctx, methodIn)
			if err != nil {
				return runtime.Executed(failedServiceCallResult(
					call,
					err,
			...
			if cfg.mapResult != nil {
				...
				result = picturesspecs.InitInspectToolResult(methodOut.(*images.ViewResult))
			}
			...
			{
				data := picturesspecs.InitInspectFixtureSharedV1ServerData(mr.Source)
				dataJSON, err := picturesspecs.InspectFixtureSharedV1ServerDataCodec().ToJSON(data)
				if err != nil {
					return runtime.Executed(failedServiceCallResult(
						call,
						err,
		...
		case tools.Ident("pictures.view"):
			...
			var serverItems []*toolregistry.ServerDataItem
			{
				data := picturesspecs.InitViewFixtureSharedV1ServerData(mr.Source)
				dataJSON, err := picturesspecs.ViewFixtureSharedV1ServerDataCodec().ToJSON(data)
				if err != nil {
					return runtime.Executed(failedServiceCallResult(
						call,
						err,
