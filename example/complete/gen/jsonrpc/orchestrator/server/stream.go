...
// RunServerStream implements the orchestrator.RunServerStream interface using
// Server-Sent Events.
type RunServerStream struct {
...
type runServerStreamEventWriter struct {
	w         http.ResponseWriter
	eventType string
	started   bool
}

func (s *runServerStreamEventWriter) Header() http.Header        { return s.w.Header() }
func (s *runServerStreamEventWriter) WriteHeader(statusCode int) { s.w.WriteHeader(statusCode) }
func (s *runServerStreamEventWriter) Write(data []byte) (int, error) {
	if !s.started {
		s.started = true
		if s.eventType != "" {
...
func (s *runServerStreamEventWriter) finish() {
	if s.started {
		s.w.Write([]byte("\n\n"))
...
func (s *RunServerStream) Send(ctx context.Context, event orchestrator.RunEvent) error {
	...
	result, ok := event.(*orchestrator.AgentRunChunk)
	if !ok {
		return fmt.Errorf("unexpected event type: %T", event)
	}
	// Convert to response body type for proper JSON encoding
	body := NewRunResponseBody(result)
...
func (s *RunServerStream) SendAndClose(ctx context.Context, event orchestrator.RunEvent) error {
...
func (s *RunServerStream) SendError(ctx context.Context, id string, err error) error {
...
func (s *RunServerStream) sendError(ctx context.Context, id any, code jsonrpc.Code, message string, data any) error {
...
// the event writer
func (s *RunServerStream) sendSSEEvent(eventType string, v any) error {
	...
	ew := &runServerStreamEventWriter{w: s.w, eventType: eventType}
