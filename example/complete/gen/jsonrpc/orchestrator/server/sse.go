...
//
// orchestrator SSE server streaming
//
// Command:
...
// orchestratorSSEStream implements the orchestrator.Stream interface for SSE
// transport.
type orchestratorSSEStream struct {
...
func (s *orchestratorSSEStream) initSSEHeaders() {
	s.once.Do(func() {
		header := s.w.Header()
...
func (s *orchestratorSSEStream) sendSSEEvent(eventType string, v any) error {
	s.initSSEHeaders()
...
func (s *orchestratorSSEStream) sendError(ctx context.Context, id any, code jsonrpc.Code, message string, data any) error {
...
func (s *orchestratorSSEStream) Send(ctx context.Context, event orchestrator.Event) error {
	switch v := event.(type) {
	case *orchestrator.AgentRunChunk:
		// Convert to response body type for proper JSON encoding
		body := NewRunResponseBody(v)
