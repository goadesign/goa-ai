...
// NewChatAssistantAssistantMcpMCPExecutor returns a ToolCallExecutor that
// proxies tool calls to an MCP caller using generated per-toolset codecs.
func NewChatAssistantAssistantMcpMCPExecutor(caller mcpruntime.Caller) runtime.ToolCallExecutor {
	...
	return runtime.ToolCallExecutorFunc(func(ctx context.Context, meta runtime.ToolCallMeta, call planner.ToolRequest) (planner.ToolResult, error) {
		...
		if pc, ok := assistant_assistant_mcp.PayloadCodec(full); ok {
			...
			if len(resp.Result) > 0 {
				if rc, ok := assistant_assistant_mcp.ResultCodec(full); ok {
