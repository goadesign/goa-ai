...
// RunClientStream implements the orchestrator.RunClientStream interface using
// Server-Sent Events.
...
func (s *RunClientStream) parseSSEEvent() (eventType string, data []byte, err error) {
...
func (s *RunClientStream) Recv(ctx context.Context) (*orchestrator.AgentRunChunk, error) {
	s.lock.Lock()
	defer s.lock.Unlock()

	var zero *orchestrator.AgentRunChunk
...
func (s *RunClientStream) decodeResult(data json.RawMessage) (*orchestrator.AgentRunChunk, error) {
	...
	var result *orchestrator.AgentRunChunk
...
func (s *RunClientStream) Close() error {
	s.lock.Lock()
	defer s.lock.Unlock()
