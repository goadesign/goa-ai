...
func EncodeRequestCompletionRequest(encoder func(*http.Request) goahttp.Encoder) func(*http.Request, any) error {
	return func(req *http.Request, v any) error {
		...
		b := NewRequestCompletionRequestBody(p)
		body := &jsonrpc.Request{
			JSONRPC: "2.0",
...
// the assistant service request_completion JSON-RPC method. restoreBody
...
func (c *Client) BuildMonitorResourceChangesRequest(ctx context.Context, v any) (*http.Request, error) {
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: MonitorResourceChangesAssistantPath()}
	req, err := http.NewRequest("POST", u.String(), nil)
	if err != nil {
...
func EncodeMonitorResourceChangesRequest(encoder func(*http.Request) goahttp.Encoder) func(*http.Request, any) error {
	return func(req *http.Request, v any) error {
		...
		b := NewMonitorResourceChangesRequestBody(p)
		body := &jsonrpc.Request{
			JSONRPC: "2.0",
			Method:  "monitor_resource_changes",
			Params:  b,
		}
		// No ID field in payload - always send as a request with generated ID
		id := uuid.New().String()
...
// returned by the assistant service monitor_resource_changes JSON-RPC method.
// restoreBody controls whether the response body should be restored after
// having been read.
...
// assistant service stream_logs JSON-RPC method. restoreBody controls whether
// the response body should be restored after having been read.
