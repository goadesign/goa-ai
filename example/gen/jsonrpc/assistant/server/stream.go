...
type processBatchServerStreamEventWriter struct {
	w         http.ResponseWriter
	eventType string
	started   bool
}

func (s *processBatchServerStreamEventWriter) Header() http.Header { return s.w.Header() }
func (s *processBatchServerStreamEventWriter) WriteHeader(statusCode int) {
	s.w.WriteHeader(statusCode)
}
func (s *processBatchServerStreamEventWriter) Write(data []byte) (int, error) {
	if !s.started {
		s.started = true
		if s.eventType != "" {
...
func (s *processBatchServerStreamEventWriter) finish() {
	if s.started {
		s.w.Write([]byte("\n\n"))
...
func (s *ProcessBatchServerStream) Send(ctx context.Context, event assistant.ProcessBatchEvent) error {
	...
	result, ok := event.(*assistant.BatchResult)
	if !ok {
		return fmt.Errorf("unexpected event type: %T", event)
	}
	// Convert to response body type for proper JSON encoding
	body := NewProcessBatchResponseBody(result)
...
func (s *ProcessBatchServerStream) SendAndClose(ctx context.Context, event assistant.ProcessBatchEvent) error {
...
func (s *ProcessBatchServerStream) SendError(ctx context.Context, id string, err error) error {
...
func (s *ProcessBatchServerStream) sendError(ctx context.Context, id any, code jsonrpc.Code, message string, data any) error {
...
// the event writer
func (s *ProcessBatchServerStream) sendSSEEvent(eventType string, v any) error {
	...
	ew := &processBatchServerStreamEventWriter{w: s.w, eventType: eventType}
