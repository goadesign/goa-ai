...
type (
	...
	RegistryClientData struct {
		...
		optionsImports []*codegen.ImportSpec
...
// registryClientFiles writes every registry client saved by the generation
// plan, including clients owned by services without agents.
func registryClientFiles(clients []*RegistryClientData) []*codegen.File {
...
func newRegistryClientData(
	...
	data := &RegistryClientData{
		...
		clientImports: importsForPaths(planned.pkg, []string{
			"context",
			"encoding/json",
			"errors",
			"fmt",
		...
		optionsImports: importsForPaths(planned.pkg, []string{
			"net/http",
			"time",
		}),
	}
...
func registryClientFile(data *RegistryClientData) *codegen.File {
	...
	sections := []*codegen.SectionTemplate{
		codegen.Header(data.GoName+" registry client", data.PackageName, data.clientImports),
		{
...
func registryClientOptionsFile(data *RegistryClientData) *codegen.File {
	...
	sections := []*codegen.SectionTemplate{
		codegen.Header(data.GoName+" registry client options", data.PackageName, data.optionsImports),
		{
