...
// ExampleMainImportNameCollisions defines agents whose package names match
// imports used directly by the generated example main program.
func ExampleMainImportNameCollisions() func() {
	return func() {
		API("example main import name collisions", func() {})
		var Result = Type("ExampleMainResult", func() {
			Attribute("text", String, "Generated completion text.")
			Required("text")
			Example(map[string]any{"text": "Generated text."})
		})
		Service("alpha", func() {
			Completion("summary", "Generates example output.", func() {
				Return(Result)
			})
			for _, name := range []string{
				"bootstrap",
				...
				Agent(name, "Exercises planned example imports.", func() {})
			}
		})
	}
}
