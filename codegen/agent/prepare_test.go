...
func TestPrepare_ForceGenerateToolTypesRecursively(t *testing.T) {
	...
	require.Equal(t, []string{"false"}, gotA.Attribute().Meta["openapi:generate"])
	_, ok = gotB.Attribute().Meta["type:generate:force"]
	require.True(t, ok, "B must be marked type:generate:force")
	require.Equal(t, []string{"false"}, gotB.Attribute().Meta["openapi:generate"])
}
...
func TestPrepare_PreservesExplicitOpenAPIGenerateMetadata(t *testing.T) {
	agentsExpr.Root = &agentsExpr.RootExpr{}
	goaexpr.Root.Types = nil

	toolType := &goaexpr.UserTypeExpr{
		AttributeExpr: &goaexpr.AttributeExpr{
			Type: goaexpr.String,
			Meta: goaexpr.MetaExpr{"openapi:generate": []string{"true"}},
		},
		TypeName: "PublicToolType",
	}
	...
	tool := &agentsExpr.ToolExpr{
		Name:    "t",
		Toolset: ts,
		Args:    &goaexpr.AttributeExpr{Type: toolType},
		Return:  &goaexpr.AttributeExpr{Type: goaexpr.Empty},
	}
	ts.Tools = []*agentsExpr.ToolExpr{tool}
	ag.Used.Toolsets = []*agentsExpr.ToolsetExpr{ts}
	...
	require.NotNil(t, got)
	require.Contains(t, got.Attribute().Meta, "type:generate:force")
	require.Equal(t, []string{"true"}, got.Attribute().Meta["openapi:generate"])
}
