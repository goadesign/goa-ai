type (
	// ToolsetInfo contains catalog metadata shared by every registry client.
	ToolsetInfo = registry.ToolsetInfo
	// ToolsetSchema contains the complete schemas consumed during discovery.
	ToolsetSchema = registry.ToolsetSchema
	// ToolSchema contains model arguments, provider arguments, and result schemas.
	ToolSchema = registry.ToolSchema
	// SearchResult describes one catalog match.
...
func (c *Client) doRequest(ctx context.Context, method, reqURL string, body []byte, result any) error {
	...
	for attempt := 0; attempt <= c.retryMax; attempt++ {
		...
		// Cancellation and permanent rejections cannot succeed by repeating
		// the same request. Timeouts and rate limits may be temporary.
		if ctx.Err() != nil {
			return ctx.Err()
		}
