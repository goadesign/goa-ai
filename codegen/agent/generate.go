...
func emitTransformsFile(agent *AgentData, ts *ToolsetData, data *toolSpecsData) *codegen.File {
	...
	for _, t := range ts.Tools {
		...
		if toolPayload != nil && t.Args != nil && t.Args.Type != goaexpr.Empty {
			srcCtx := codegen.NewAttributeContextForConversion(false, false, true, ts.SpecsPackageName, codegen.NewNameScope())
			tgtCtx := codegen.NewAttributeContext(false, false, false, svcAlias, codegen.NewNameScope())
		...
		if toolResult != nil && t.Return != nil && t.Return.Type != goaexpr.Empty {
			srcCtx := codegen.NewAttributeContext(false, false, false, svcAlias, codegen.NewNameScope())
	...
	sections := []*codegen.SectionTemplate{
		...
		{Name: "tool-transforms", Source: agentsTemplates.Read(toolTransformsFileT), Data: transformsFileData{Functions: fns}},
	}
