...
func (s *subscription) C() <-chan *ToolsCallResult { return s.ch }
...
func (a *MCPAdapter) ResourcesRead(ctx context.Context, p *ResourcesReadPayload) (*ResourcesReadResult, error) {
    if !a.isInitialized() { return nil, goa.PermanentError("invalid_params", "Not initialized") }
    if err := a.assertResourceURIAllowed(ctx, p.URI); err != nil { return nil, goa.PermanentError("invalid_params", "%s", err.Error()) }
...
{{- range .Resources }}
    case {{ quote .URI }}:
        ...
        result, err := a.service.{{ .OriginalMethodName }}(ctx)
		{{- end }}
		if err != nil { return nil, a.mapError(err) }
		s, serr := encodeJSONToString(ctx, result)
		if serr != nil { return nil, goa.PermanentError("invalid_params", "%s", serr.Error()) }
		...
		{{- else }}
		{{- if .HasPayload }}
		if err := a.service.{{ .OriginalMethodName }}(ctx, payload); err != nil { return nil, a.mapError(err) }
		{{- else }}
		if err := a.service.{{ .OriginalMethodName }}(ctx); err != nil { return nil, a.mapError(err) }
		{{- end }}
...
func (a *MCPAdapter) EventsStream(ctx context.Context, stream ToolsCallServerStream) error {
    if !a.isInitialized() { return goa.PermanentError("invalid_params", "Not initialized") }
    if a.broadcaster == nil { return goa.PermanentError("invalid_params", "No broadcaster configured") }
...
func (a *MCPAdapter) Publish(ev *ToolsCallResult) {
    if a == nil || a.broadcaster == nil { return }
