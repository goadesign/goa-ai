...
func (p *mcpPlugin) generate(plan *goagenerator.Plan, files []*goacodegen.File) ([]*goacodegen.File, error) {
	for _, planned := range p.planned {
		...
		if err := bindMCPServiceMethods(services, planned, mcpService); err != nil {
			return nil, err
		}
...
// bindMCPServiceMethods copies the final payload type used by the internal
// initialized notification after Goa has chosen its package and type names.
func bindMCPServiceMethods(services *goaservice.ServicesData, planned *plannedMCPService, service *goaservice.Data) error {
	...
	attributor := services.ServiceAttributor(service.Name, planned.adapterData.jsonrpcClientImportPath)
	planned.adapterData.ClientSession.InitializedPayloadType = attributor.Name(methodExpr.Payload, "", false, false)
	return nil
}
