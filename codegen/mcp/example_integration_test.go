...
func TestMCPStaticPromptExampleUsesFinalConstructorNames(t *testing.T) {
...
// configureCollidingExampleDesign registers two services whose names need
// different Go constructors. The first service includes a static prompt so the
// generated example must call the adapter constructor that embeds that prompt.
	...
	mcpexpr.Root.RegisterMCP(first, &mcpexpr.MCPExpr{
		...
		Prompts: []*mcpexpr.PromptExpr{{
			...
			Messages: []*mcpexpr.MessageExpr{{
				Role:    "user",
				Content: "Summarize the current value.",
			}},
		}},
