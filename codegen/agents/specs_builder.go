...
func (d *toolSpecsData) codecsImports() []*codegen.ImportSpec {
	base := []*codegen.ImportSpec{
		...
		codegen.SimpleImport("goa.design/goa-ai/runtime/agents/tools"),
	}
...
func (b *toolSpecBuilder) importsForTool(t *ToolData) map[string]*codegen.ImportSpec {
	if len(t.Toolset.SourceServiceImports) > 0 {
		return t.Toolset.SourceServiceImports
	}
	return b.svcImports
}
...
func (b *toolSpecBuilder) typeFor(tool *ToolData, att *goaexpr.AttributeExpr, usage typeUsage) (*typeData, error) {
	...
	info, err := b.ensureType(tool, att, usage)
	if err != nil {
		return nil, err
	}
	return info, nil
...
func (b *toolSpecBuilder) ensureType(tool *ToolData, att *goaexpr.AttributeExpr, usage typeUsage) (*typeData, error) {
...
func (b *toolSpecBuilder) materialize(
	...
	// Simplified: never synthesize local types. Always reference owner types or primitives.
	if att == nil || att.Type == nil || att.Type == goaexpr.Empty {
		// Should not happen due to Prepare, but handle gracefully.
		return att, "", "", nil
	}
	// Compute full reference
	if ut, ok := att.Type.(goaexpr.UserType); ok && ut != nil {
		pkg := packageName(codegen.UserTypeLocation(ut), svc)
		fullRef = scope.GoFullTypeRef(&goaexpr.AttributeExpr{Type: ut}, pkg)
	} else {
		// Primitive or composite: use direct ref
		fullRef = scope.GoTypeRef(att)
	}
	tt = att
	// Imports from attribute metadata and locations
	imports = gatherAttributeImports(b.genpkg, tt)
	// Ensure service import for service-local user types (no explicit location)
	if referencesServiceUserType(tt) && svc != nil && svc.PathName != "" {
		svcAlias := servicePkgAlias(svc)
		svcPath := joinImportPath(b.genpkg, svc.PathName)
		if svcPath != "" {
			imports = append(imports, &codegen.ImportSpec{Name: svcAlias, Path: svcPath})
		}
	}
	// No type definition emitted.
	return tt, "", fullRef, imports
}
...
// cloneToLocalTypes removed: we no longer synthesize local types for tools.
...
// referencesServiceUserType reports whether the attribute references a user
// type that belongs to the service package (i.e., lacks an explicit location).
func referencesServiceUserType(att *goaexpr.AttributeExpr) bool {
	if att == nil || att.Type == nil || att.Type == goaexpr.Empty {
		return false
	}
	found := false
